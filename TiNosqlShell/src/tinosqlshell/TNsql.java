...
//    private final static String env = "dev";
    ...
    public static void main(String argv[]) throws Exception {
        ...
        if (env.equals("deploy") && console == null && !GraphicsEnvironment.isHeadless()) {
            String filename = TNsql.class.getProtectionDomain().getCodeSource().getLocation().toString().substring(6);
            try {
            ...
            while (true) {
                ...
                if (keywordInternal.contains(sentence_to_server)
                    ...
                    if (sentence_to_server.equals("ping")) {
                        TNsql.startPingTime = System.currentTimeMillis();
                        sendRequest(sentence_to_server + " " + TNsql.startPingTime);
                    }
                    if (sentence_to_server.startsWith("connect")) {
                        ...
                        if (keywords.length == 3) {
                            ...
                            TNsql.startPingTime = System.currentTimeMillis();
                            sendRequest("ping " + TNsql.startPingTime);
